...
Columbia = ['Columbia University', 'Manhattan, NY', 'Bronx, NY', 'North Bergen, NJ']
...
fixers = ['Brooklyn, NY', 'Queens, NY']

location_list = NU+UChicago+Columbia+Stanford+JHU+Vanderbilt+Madison+CollegePark+UCLA+WashU+Rice
def main():
	# Title text
			...
			# 	self.handle = None
			    ...
			    with open('all_yaks.csv', 'wt') as f:
			        ...
			        # update our location to the current school
			            coordlocation = changeLocation(geocoder, location)                            
			            remoteyakker.update_location(coordlocation)
			            for i in remoteyakker.get_yaks():
			                writer = csv.writer(f)
			                # write out whatever values you desire
			                writer.writerow([i.message_id, i.poster_id, location, coordlocation, str(i.message), i.latitude, i.longitude, i.time, i.likes, i.comments])
